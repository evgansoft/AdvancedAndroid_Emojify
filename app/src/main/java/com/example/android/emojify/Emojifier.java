...
class Emojifier {
     ...
     */
    static Bitmap detectFacesAndIverlayEmoji(Context context, Bitmap picture) {
        ...
        if(faces.size() == 0) {
            ...
            for (int i = 0; i < faces.size(); ++i) {
                ...
                final Emoji emoji = whichEmoji(face);
                ...
                switch (emoji) {
                    case CLOSED_EYE_FROWN:
                        emojiBitmap = BitmapFactory.decodeResource(
                                resources, R.drawable.closed_frown);
                        break;
                    case CLOSED_EYE_SMILE:
                        emojiBitmap = BitmapFactory.decodeResource(
                                resources, R.drawable.closed_smile);
                        break;
                    case FROWN:
                        emojiBitmap = BitmapFactory.decodeResource(
                                resources, R.drawable.frown);
                        break;
                    case LEFT_WINK:
                        emojiBitmap = BitmapFactory.decodeResource(
                                resources, R.drawable.leftwink);
                        break;
                    case LEFT_WINK_FROWN:
                        emojiBitmap = BitmapFactory.decodeResource(
                                resources, R.drawable.leftwinkfrown);
                        break;
                    case RIGHT_WINK:
                        emojiBitmap = BitmapFactory.decodeResource(
                                resources, R.drawable.rightwink);
                        break;
                    case RIGHT_WINK_FROWN:
                        emojiBitmap = BitmapFactory.decodeResource(
                                resources, R.drawable.rightwinkfrown);
                        break;
                    case SMILE:
                        emojiBitmap = BitmapFactory.decodeResource(
                                resources, R.drawable.smile);
                        break;
                    default:
                        throw new IllegalArgumentException("Unknown emoji");
                }
    ...
    private static Bitmap addBitmapToFace(final Bitmap backgroundBitmap, final Bitmap emojiBitmap, final Face face) {
        ...
        // Scale the emoji
        Bitmap scaledEmojiBitmap = Bitmap.createScaledBitmap(emojiBitmap, newEmojiWidth, newEmojiHeight, false);

        // Determine the emoji position so it best lines up with the face
        float emojiPositionX =
                (face.getPosition().x + face.getWidth() / 2) - scaledEmojiBitmap.getWidth() / 2;
        float emojiPositionY =
                (face.getPosition().y + face.getHeight() / 2) - scaledEmojiBitmap.getHeight() / 3;

        // Create the canvas and draw the bitmaps to it
        Canvas canvas = new Canvas(resultBitmap);
        canvas.drawBitmap(backgroundBitmap, 0, 0, null);
        canvas.drawBitmap(scaledEmojiBitmap, emojiPositionX, emojiPositionY, null);
